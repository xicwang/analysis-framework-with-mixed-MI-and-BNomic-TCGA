...
class search:
        ...
        def reverse_p_score(self,cnode):
                """ Simple reverse search that looks for an edge to remove ** """
                family=[cnode]+self.BN.pnodes[cnode]
                score=self.scores[cnode]
                old_score=score 
                parent=[]
        ...
        def add_edge_and_sync(self,cnode,pnode):
                """ Add the edge to BN, apply the induced constraints and update scores
                """
        ...
        def remove_edge_and_sync(self,cnode,pnode):
                """ Remove the edge from BN relax constraints and update the scores sss"""
                d_set=self.BN.find_descendants1(cnode)#|set([cnode])
...
#                       for j in self.BN.pconstraints[i]&d_set
#                               :#d_set-self.BN.p_candidates[i]:
#                               subset=family+[j]
#                               new_score=self.objfunc(self.data[:,subset],self.arity[subset])
#
#                               self.delta_cache[i,j]=new_score-self.scores[i]  
#                       #self.BN.p_candidates[i]|=d_set
#
